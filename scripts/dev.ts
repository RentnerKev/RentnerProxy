...
export interface DevelopmentDependencies {
  ...
  signalProcessTree(childProcess: ManagedProcess, signal: NodeJS.Signals): void
...
interface ServiceDefinition {
  label: string
  script: 'dev:web' | 'dev:controller'
}

interface RunningService extends ServiceDefinition {
...
export async function runDevelopment(
  ...
  async function runSession(): Promise<number> {
    ...
    try {
      ...
      dependencies.logger.fail(`Development process failed to start: ${formatSpawnError(error)}`)
