                        ...
                        {week.map((day) => (
                            <td key={day.dateValue} className="w-[14.2857%] p-px">
                                <button
                                    ...
                                    className={`relative grid h-12 w-full min-w-0 place-items-center rounded-lg text-xs font-bold outline-hidden transition-[background-color,color,box-shadow] focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-1 focus-visible:ring-offset-surface-raised motion-reduce:transition-none ${day.isRangeStart || day.isRangeEnd ? 'bg-brand-500 text-navy-950' : day.isInRange ? 'rounded-sm bg-success-bg text-success-text' : day.isCurrentMonth ? 'text-ink-soft hover:bg-surface-hover hover:text-brand-text' : 'text-muted-soft/55 hover:bg-surface-hover hover:text-muted'} ${day.isToday && !day.isRangeStart && !day.isRangeEnd ? 'ring-1 ring-brand-600/45' : ''}`}
                                >
                                    {day.dayNumber}
                                </button>
                            </td>
